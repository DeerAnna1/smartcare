		...
		<div className="p-8">
			<div className="mb-6">
				<div className="flex items-center gap-2 text-sm text-on-surface-variant mb-2">
					...
					<span className="text-on-surface font-semibold">历史会话</span>
				</div>
				<div>
					<h1 className="font-headline font-bold text-3xl text-on-surface">历史会话</h1>
					<p className="text-on-surface-variant text-sm mt-1">
						点击会话可直接进入对应执行控制台；未归档事件可在列表中直接归档到健康档案。
					</p>
				</div>
			</div>
			...
			<div className="grid grid-cols-3 gap-4 mb-6">
				<div className="bg-surface-container-lowest rounded-2xl p-6 border-l-4 border-primary shadow-sm">
					<p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-2">历史事件数</p>
					<p className="font-headline font-bold text-4xl text-primary">{events.length}</p>
					<p className="text-xs text-on-surface-variant mt-2">全部健康事件</p>
				</div>
				<div className="bg-surface-container-lowest rounded-2xl p-6 border-l-4 border-secondary shadow-sm">
					<p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-2">已归档</p>
					<p className="font-headline font-bold text-4xl text-secondary">{archivedCount}</p>
					<p className="text-xs text-on-surface-variant mt-2">已进入健康档案</p>
				</div>
				<div className="bg-surface-container-lowest rounded-2xl p-6 border-l-4 border-tertiary shadow-sm">
					<p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-2">待归档</p>
					<p className="font-headline font-bold text-4xl text-tertiary">{Math.max(events.length - archivedCount, 0)}</p>
					<p className="text-xs text-on-surface-variant mt-2">可手动归档</p>
				</div>
			</div>

			<div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/10 shadow-sm overflow-hidden">
				<div className="p-6 border-b border-outline-variant/10">
					<h2 className="font-headline font-bold text-on-surface">会话列表</h2>
				</div>

				{loading ? (
					<div className="p-10 text-center text-on-surface-variant">加载中...</div>
				) : events.length === 0 ? (
					<div className="p-10 text-center text-on-surface-variant">暂无历史会话</div>
				) : (
							...
							return (
								<div key={event.event_id} className="flex items-center gap-5 px-6 py-4 hover:bg-surface-container-low transition-all">
									...
									>
										...
										<div className="flex-1">
											<div className="flex items-center gap-3">
												...
												<p className="text-xs text-on-surface-variant">{event.recommended_department || "未分科"}</p>
												<p className="text-xs text-on-surface-variant">{event.triage_level}</p>
											</div>
											<p className="font-semibold text-on-surface mt-0.5">{event.chief_complaint || event.triage_level || "健康事件"}</p>
										</div>
									</button>
									...
									<div className="flex items-center gap-2 shrink-0">
										...
										>
											{event.archived ? "已归档" : isArchiving ? "归档中..." : "归档"}
										</button>
									</div>
								</div>
							);
